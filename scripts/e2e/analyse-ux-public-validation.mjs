...
const EXPECTED_BUNDLE = "index-k5RnvNRS.js";
const VIEWPORTS = [
...
async function openAnalysePage(cdp, viewport) {
  await cdp.send("Emulation.setDeviceMetricsOverride", viewport);
  await cdp.send("Page.navigate", { url: `${PUBLIC_URL}?analyseUxValidation=${viewport.name}-${Date.now()}` });
  await waitFor(cdp, "document.readyState === 'complete'", `${viewport.name} load`);
  await waitFor(cdp, "/HORIZON|SOLDE ACTUEL|Analyse/i.test(document.body?.innerText || '')", `${viewport.name} app shell`);

  await evaluate(cdp, `
    (() => {
      const entries = [...document.querySelectorAll('button, a, [role="tab"], [role="button"]')];
      let target = entries.find((entry) => /^Analyse$/i.test((entry.textContent || '').trim()))
        || entries.find((entry) => /Analyse/i.test(entry.textContent || ''));
      if (!target) {
        const more = entries.find((entry) => /^Plus$/i.test((entry.textContent || '').trim()))
          || entries.find((entry) => /Plus/i.test(entry.textContent || ''));
        if (more) more.click();
      }
  ...
  await evaluate(cdp, `
    (() => {
      const entries = [...document.querySelectorAll('button, a, [role="tab"], [role="button"]')];
      const target = entries.find((entry) => /^Analyse$/i.test((entry.textContent || '').trim()))
        || entries.find((entry) => /Analyse/i.test(entry.textContent || ''));
      if (target) target.click();
      return Boolean(target);
    })()
  `);
  await waitFor(cdp, "/Analyse financiere|Synthese/i.test(document.body?.innerText || '')", `${viewport.name} analyse page`);
}

async function validateViewport(cdp, viewport) {
  await openAnalysePage(cdp, viewport);
  await waitFor(cdp, "!/Chargement de l'analyse en cours/i.test(document.body?.innerText || '')", `${viewport.name} analyse loaded`);

  const result = await evaluate(cdp, `
    (() => {
      const text = document.body.innerText || '';
      const checks = [
        { label: 'Synthese', pattern: /Synthese/i },
        { label: 'Cartes KPI', pattern: /Cartes KPI/i },
        { label: 'Graphiques', pattern: /Graphiques/i },
        { label: 'Classements', pattern: /Classements/i },
        { label: 'Details', pattern: /Details/i },
        { label: 'Depenses', pattern: /Depenses/i },
        { label: 'Revenus', pattern: /Revenus/i },
        { label: 'Epargne', pattern: /Epargne/i },
      ];
      const missing = checks.filter((check) => !check.pattern.test(text)).map((check) => check.label);
      const progressBars = document.querySelectorAll('[role="progressbar"]').length;
      const chartSvgs = document.querySelectorAll('svg[role="img"]').length;
      const buttons = [...document.querySelectorAll('button')].filter((entry) => /Depenses variables|Frais fixes|Revenus fixes|Revenus variables/i.test(entry.innerText || '')).length;
      const activeFilterChips = /Compte:|Categorie:|Precedent:/i.test(text);
  ...
  if (result.missing.length) throw new Error(`${viewport.name}: missing analyse labels ${result.missing.join(", ")}`);
  if (result.progressBars < 1) throw new Error(`${viewport.name}: ranking progress bars missing`);
  if (result.chartSvgs < 1) throw new Error(`${viewport.name}: chart SVGs missing`);
  if (result.rankingButtons < 1) throw new Error(`${viewport.name}: ranking cards missing`);
  if (!result.activeFilterChips) throw new Error(`${viewport.name}: active filter chips missing`);
...
async function main() {
  ...
  const userDataDir = await mkdtemp(join(tmpdir(), "horizon-analyse-ux-"));
