...
async function createAdminDb() {
  ...
  assertEmulatorWriteMode({ operationName: "maintenance:verify-mass-classification-controlled" });
  assertAutomatedWriteAllowed({
    projectId: serviceAccount.project_id,
    operationName: "maintenance:verify-mass-classification-controlled",
  });
...
async function pickTwoControlledTransactions(db) {
  ...
  const candidates = snapshot.docs
    ...
    .filter((tx) => tx.montant !== undefined)
...
async function pickDifferentCategory(db, transactions) {
  ...
  assert(categories.length > 0, "Aucune categorie active disponible");

  const excluded = new Set(transactions.map((tx) => String(tx.categoryId || "").trim()).filter(Boolean));
  const fallbackCurrent = new Set(transactions.map((tx) => normalizeCategoryName(tx)).filter(Boolean));

  const target = categories.find((category) => !excluded.has(String(category.id).trim()))
    || categories.find((category) => !fallbackCurrent.has(String(category.name || "").trim()))
    || categories[0];

  assert(target?.id, "Categorie cible introuvable");
  return target;
}
...
async function main() {
  ...
  const initialTransactions = await pickTwoControlledTransactions(db);
  ...
  const targetCategory = await pickDifferentCategory(db, initialTransactions);

  const patchApplied = { categoryId: targetCategory.id };
...
main().catch((error) => {
  console.error("CONTROLLED_CLASSIFICATION_TEST_FAILED");
