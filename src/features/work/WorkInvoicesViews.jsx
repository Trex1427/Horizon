...
import { Alert, Box, Button, Card, CardActions, CardContent, Chip, CircularProgress, Dialog, DialogActions, DialogContent, DialogTitle, MenuItem, Stack, TextField, Typography, useMediaQuery } from "@mui/material";
...
function InvoiceDialog({ draft, extraction, file, thirdParties, projects, activities, addThirdParty, createProject, onClose, onSave }) {
  ...
  return <Dialog open fullWidth fullScreen={isMobile} maxWidth="sm" scroll="paper" aria-labelledby="invoice-import-title" onClose={onClose} PaperProps={{ sx: { overflowX: "hidden" } }}><DialogTitle id="invoice-import-title">Valider la facture Tiiime</DialogTitle><DialogContent><Stack spacing={2} sx={{ pt: 1 }}>
  ...
  </Stack></DialogContent><DialogActions sx={{ position: "sticky", bottom: 0, bgcolor: "background.paper", borderTop: "1px solid", borderColor: "divider", px: 2, pb: { xs: 2, sm: 1 } }}><Button onClick={onClose} disabled={saving}>Annuler</Button><Button variant="contained" onClick={submit} disabled={saving}>{saving ? <CircularProgress size={20}/> : "Importer"}</Button></DialogActions>
...
export function WorkInvoicesSection({ invoices, projects, thirdParties, activities, accounts, addThirdParty, createProject, thirdPartyMap, projectMap, loading, error, parsePdf, importInvoice, markPaid, markPaidWithTransaction, markPending, inspectDelete, deleteInvoice, openPdf }) {
  ...
  return <>{notice && <Alert sx={{ mb: 2 }} severity="info" onClose={() => setNotice("")}>{notice}</Alert>}<Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" spacing={1} sx={{ mb: 2 }}><Typography variant="h6">Factures</Typography><Button component="label" variant="contained" startIcon={importing ? <CircularProgress size={18}/> : <UploadFile/>} disabled={importing}>Importer une facture PDF<input ref={inputRef} hidden type="file" accept="application/pdf" onChange={(e) => selectPdf(e.target.files?.[0])}/></Button></Stack>
    <Box component="details" open={!isMobile} sx={{ mb: 2, border: { xs: "1px solid", md: 0 }, borderColor: "divider", borderRadius: 2, p: { xs: 1, md: 0 } }}><Box component="summary" sx={{ display: { xs: "flex", md: "none" }, minHeight: 44, alignItems: "center", fontWeight: 700 }}>Filtres</Box><Stack direction={{ xs: "column", md: "row" }} spacing={1} sx={{ mt: { xs: 1, md: 0 } }}><TextField select size="small" label="Dossier" value={filters.project} onChange={(e) => setFilters({ ...filters, project: e.target.value })}><MenuItem value="all">Tous</MenuItem>{projects.map((entry) => <MenuItem key={entry.id} value={entry.id}>{entry.name}</MenuItem>)}</TextField><TextField select size="small" label="Client" value={filters.client} onChange={(e) => setFilters({ ...filters, client: e.target.value })}><MenuItem value="all">Tous</MenuItem>{thirdParties.map((entry) => <MenuItem key={entry.id} value={entry.id}>{entry.name}</MenuItem>)}</TextField><TextField select size="small" label="Statut" value={filters.status} onChange={(e) => setFilters({ ...filters, status: e.target.value })}><MenuItem value="all">Tous</MenuItem>{Object.entries(WORK_INVOICE_STATUS_LABELS).map(([value,label]) => <MenuItem key={value} value={value}>{label}</MenuItem>)}</TextField><Button onClick={() => setFilters({ project: "all", client: "all", status: "all" })} sx={{ minHeight: 44 }}>Réinitialiser</Button></Stack></Box>
    {loading ? <CircularProgress/> : error ? <Alert severity="error">{error}</Alert> : <Stack spacing={1}>{filtered.map((invoice) => <Card key={invoice.id} variant="outlined" role="button" tabIndex={0} onClick={() => openPdf(invoice)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); openPdf(invoice); } }} sx={{ cursor: "pointer" }}><CardContent><Box sx={{ display: "grid", gridTemplateColumns: { xs: "minmax(0, 1fr)", md: "1.2fr 1fr 1fr 1fr auto" }, gap: 1 }}><Typography fontWeight={700}>{invoice.invoiceNumber || "Sans numéro"}</Typography><Typography>{invoice.invoiceDate || "Date absente"}</Typography><Typography>{thirdPartyMap.get(invoice.thirdPartyId)?.name || "Client indisponible"}</Typography><Typography>{projectMap.get(invoice.workProjectId)?.name || "Sans dossier"}</Typography><Stack direction="row" spacing={1}><Typography fontWeight={700}>{currency.format(Number(invoice.amountTTC || 0))}</Typography><Chip size="small" label={WORK_INVOICE_STATUS_LABELS[invoice.status] || invoice.status}/></Stack></Box></CardContent><CardActions sx={{ flexWrap: "wrap", "& .MuiButtonBase-root": { minHeight: 44 } }}><Button startIcon={<Description/>} onClick={(e) => { e.stopPropagation(); openPdf(invoice); }}>PDF</Button>{invoice.status === "pending_payment" && <Button startIcon={<Paid/>} disabled={paymentSubmitting} onClick={async (e) => {
  ...
  setNotice(result.success ? "Facture supprimée. Le PDF est conservé." : result.error);
    ...
    <Dialog open={Boolean(deletionInvoice)} fullScreen={isMobile} scroll="paper" onClose={() => { if (!deletionSubmitting) setDeletionInvoice(null); }} fullWidth maxWidth="sm">
      ...
      <DialogActions sx={{ flexWrap: "wrap", position: "sticky", bottom: 0, bgcolor: "background.paper", borderTop: "1px solid", borderColor: "divider", "& .MuiButtonBase-root": { minHeight: 44 } }}>
    ...
    <Dialog open={Boolean(paymentInvoice)} fullScreen={isMobile} scroll="paper" onClose={() => { if (!paymentSubmitting) setPaymentInvoice(null); }} fullWidth maxWidth="xs">
    ...
    <Dialog open={Boolean(pendingInvoice)} fullScreen={isMobile} scroll="paper" onClose={() => { if (!paymentSubmitting) setPendingInvoice(null); }} fullWidth maxWidth="xs">
