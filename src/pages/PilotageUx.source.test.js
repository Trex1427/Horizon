...
test("Budgets and Objectifs keep existing actions while adding relevant search", async () => {
  ...
  assert.equal(budgets.includes("Rechercher un budget"), true);
  assert.equal(objectifs.includes("Rechercher un objectif"), true);
  assert.equal(budgets.includes("onClick={() => {") || budgets.includes("onAdd={() => {"), true);
  assert.equal(objectifs.includes("onAdd={() => {") || objectifs.includes("onClick={() => {"), true);
  ...
  assert.equal(budgets.includes("TransactionUsageExplorer"), true);
});
...
test("Pilotage cards use the shared progress style and accessible progress labels", async () => {
  ...
  assert.equal(budgetCard.includes("bgcolor: \"rgba(23, 42, 47, 0.08)\""), true);
