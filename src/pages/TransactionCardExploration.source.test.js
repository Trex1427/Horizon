...
test("App passes the transaction-opening callback into Budgets and Frais fixes", async () => {
  const app = await readFile(appPath, "utf8");

  assert.equal(app.includes('<FraisFixes onOpenTransactionsFiltered={openTransactionsWithContext} />'), true);
  assert.equal(app.includes('<Budgets accounts={accounts} onOpenTransactionsFiltered={openTransactionsWithContext} />'), true);
});
