...
test("fixed-expense creation checks compatible active documents and writes directly", async () => {
  ...
  assert.equal(content.includes("buildFixedExpenseDocumentId"), false);
  assert.equal(content.includes("await runTransaction"), false);
  assert.equal(content.includes('error.code = "fixed-expense/already-exists"'), true);
  assert.equal(content.includes("addDoc"), true);
});
